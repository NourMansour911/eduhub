...
class Retrieval:
		...
		for weight, results in zip(weights, result_lists):
			for rank, item in enumerate(results, start=1):
				...
				if doc_id not in fused_scores:
					fused_scores[doc_id] = {"rrf_score": 0, "doc": item}
	...
	def _reciprocal_rank_fusion_across_lists(
		...
		ordered_ids = sorted(fused_scores.keys(), key=lambda item: fused_scores[item], reverse=True)
		return [docs_by_id[item] for item in ordered_ids[:top_k]]
